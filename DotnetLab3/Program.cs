...
for (var i = 2; i < 15; i++)
{
    ...
    multiThreadedResults.Add(new BenchmarkRow(threads, "Multi-threaded",
        Measure(() => Matrix.Multiply(a, b, threads))));
    lowLevelResults.Add(new BenchmarkRow(threads, "Multi-threaded Low-Level",
        Measure(() => Matrix.MultiplyLowLevel(a, b, threads))));
}
...
static void PrintTable(IReadOnlyList<BenchmarkRow> results)
{
    ...
    var millisecondsWidth = Math.Max(millisecondsHeader.Length,
        results.Max(r => r.Elapsed.TotalMilliseconds.ToString("0.000").Length));
    ...
    Console.WriteLine(
        $"| {threadsHeader.PadRight(threadsWidth)} | {benchmarkHeader.PadRight(benchmarkWidth)} | {millisecondsHeader.PadLeft(millisecondsWidth)} | {elapsedHeader.PadRight(elapsedWidth)} |");
    Console.WriteLine(Separator(threadsWidth, benchmarkWidth, millisecondsWidth, elapsedWidth));

    foreach (var row in results)
        Console.WriteLine(
            $"| {row.Threads.ToString().PadRight(threadsWidth)} | {row.Name.PadRight(benchmarkWidth)} | {row.Elapsed.TotalMilliseconds.ToString("0.000").PadLeft(millisecondsWidth)} | {row.Elapsed.ToString().PadRight(elapsedWidth)} |");
